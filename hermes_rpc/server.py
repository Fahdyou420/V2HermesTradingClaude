...
# Fix python path for shared modules
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
try:
    from services.shared.error_bus import publish_error
except ImportError:
    # Fallback to prevent startup failures on raw Windows environments where services module is not added
    ...
    async def sse_event_generator():
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            try:
                ...
                publish_error("hermes_rpc", "ERROR", "Ollama connection failure during streaming", str(exc))
                yield f"data: [Host Ollama Error] {str(exc)}\n\n"
            except Exception as exc:
                logger.error(f"Unmanaged exception in token stream: {exc}")
                publish_error("hermes_rpc", "ERROR", "Unmanaged exception in Ollama token stream", str(exc))
